//************* Modified by Gaspard QIN in lines 510-520
...
public:
	 ...
	 */
	processors( size_t count,T *ObjOfs, processor_listener_ptr listener )
	{
		assert( count > 0 );
		assert( listener );
		for( size_t n = 0; n < count; ++n )
		{
			//************* Modified by Gaspard QIN
			processor_ptr processor( boost::make_shared< processor< T > >( n, ObjOfs[n], boost::ref( m_indQueue ), listener ) );
